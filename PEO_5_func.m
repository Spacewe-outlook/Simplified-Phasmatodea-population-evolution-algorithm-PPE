function [Gbest,Fit_min,fit_PEO]= PEO_5_func(fhd,Dimension,Particle_Number,Max_Gen,VRmin,VRmax,varargin)
...
for t = 2:Max_Gen
    ...
    for p = 1:Np
        ...
        if new_fitness(p) <= fitness(p)
            ...
            % 更新繁殖速率
            Pa(p) = Pa(p)*(1+ (fitness(p)- new_fitness(p)) /fitness(p)); % 变好了就增加，否则减少
            %计算新的种群数量
            Px(p) = Pa(p)*Px(p)*(1-Px(p));
            %计算新的种群演化趋势
            A1 = choosebest(L_pha,Pha(p,:)); % 趋同进化，最近最优
            A3 = tubian(Dim,sigma);
            A(p,:) = (1-Px(p)).*A1.*0.2+ Px(p).*A(p,:) + 2*Px(p).*A3; % 逐步收缩到最优区域
            %         A(p,:) = (1-Px(p)).*A1.*0.1+ Px(p).*A(p,:) + (1/Dim).*A3;
        else
            ...
            stepsize = (1/t)^(abs(new_fitness(p)- new_fit_best)/(new_fit_best - new_fit_worst));

            A(p,:) = rand(1,Dim).*A1+TT*stepsize.*randn(1,Dim); 
            TT = TT*0.99;
        end
